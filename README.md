# 🚀 Deep Packet Inspection (DPI) Engine in C++

A high-performance C++ system for analyzing network traffic using **Deep Packet Inspection (DPI)**.  
This project parses raw packets, extracts TLS SNI, classifies applications, and applies rule-based filtering using a scalable multi-threaded architecture.

---

## ⚡ Key Features

- 🔍 Deep Packet Inspection (TCP, HTTP, TLS)
- 🌐 SNI extraction from TLS Client Hello
- 📦 Flow tracking using 5-tuple (stateful inspection)
- 🚫 Rule-based blocking (IP, domain, application)
- ⚙️ Multi-threaded pipeline (Load Balancer + Fast Path workers)
